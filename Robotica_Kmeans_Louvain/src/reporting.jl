...
# ── Sección Louvain (apéndice a reporte.md) ─────────────────────────────────────

"""
    append_louvain_section(output_dir; louvain_result, knn_k, compare_df,
                           agreement_nt, n_train) -> String

Añade al final de reporte.md la sección de Louvain y la comparación con K-Means.
"""
function append_louvain_section(
    output_dir   ::String;
    louvain_result,
    knn_k        ::Int,
    ...
    open(path, "a") do io
        ...
        println(io, "---")
        println(io, "")
        println(io, "## 7. Detección de Comunidades con Louvain")
        println(io, "")
        println(io, "Además de K-Means se aplicó el **algoritmo de Louvain** sobre un **grafo k-NN**")
        println(io, "construido a partir de las mismas features. Mientras K-Means parte el espacio")
        println(io, "por distancia a centroides (y exige fijar K), Louvain **descubre el número de")
        println(io, "comunidades automáticamente** maximizando la *modularidad* Q.")
        println(io, "")
        println(io, "### 7.1 Construcción del grafo")
        println(io, "")
        println(io, "| Parámetro | Valor |")
        println(io, "|-----------|-------|")
        println(io, "| Nodos (actuadores train) | $n_train |")
        println(io, "| Vecinos k-NN (k) | $knn_k |")
        println(io, "| Peso de arista | kernel gaussiano `exp(-d²/2σ²)`, σ adaptativo |")
        println(io, "| Grafo | no dirigido, simetrizado por OR |")
        println(io, "")
        println(io, "### 7.2 Modularidad")
        println(io, "")
        println(io, "```")
        println(io, "Q = (1/2m) · Σ_ij [ W_ij − k_i·k_j/2m ] · δ(c_i, c_j)")
        println(io, "```")
        println(io, "")
        @printf(io, "**Comunidades detectadas: %d**   |   **Modularidad Q = %.4f**\n",
                louvain_result.n_communities, louvain_result.Q)
        println(io, "")
        println(io, "Evolución multinivel:")
        println(io, "")
        println(io, "| Nivel | Comunidades | Q |")
        println(io, "|:---:|:---:|:---:|")
        for h in louvain_result.history
            @printf(io, "| %d | %d | %.4f |\n", h.level, h.n_comm, h.Q)
        end
        println(io, "")
        println(io, "![Grafo Louvain](animations/louvain_grafo.gif)")
        println(io, "")
        println(io, "![Comunidades Louvain (PNG)](report/louvain_comunidades.png)")
        println(io, "")
        println(io, "### 7.3 Comparación K-Means vs Louvain")
        println(io, "")
        println(io, "Ambos métodos evaluados contra el ground truth sintético (3 estados de mantenimiento):")
        println(io, "")
        println(io, "| Método | Nº Clusters | Purity vs GT | ARI vs GT | NMI vs GT | Métrica propia |")
        println(io, "|--------|:---:|:---:|:---:|:---:|---|")
        for r in eachrow(compare_df)
            @printf(io, "| %s | %d | %.3f | %.3f | %.3f | %s |\n",
                r.metodo, r.n_clusters, r.purity_vs_gt,
                r.ari_vs_gt, r.nmi_vs_gt, r.metrica_propia)
        end
        println(io, "")
        println(io, "**Acuerdo directo K-Means ↔ Louvain** (sin ground truth):")
        println(io, "")
        @printf(io, "- ARI = %.3f   |   NMI = %.3f\n", agreement_nt.ari, agreement_nt.nmi)
        println(io, "")
        println(io, "![Comparación lado a lado](report/comparacion_kmeans_louvain.png)")
        println(io, "")
        println(io, "### 7.4 Interpretación")
        println(io, "")
        println(io, "| Aspecto | K-Means | Louvain |")
        println(io, "|---------|---------|---------|")
        println(io, "| Paradigma | Particional (geométrico) | Comunidades en grafo |")
        println(io, "| ¿Requiere K? | Sí (auto-selección por silhouette) | No — K emerge de Q |")
        println(io, "| Optimiza | Inercia (WCSS) | Modularidad Q |")
        println(io, "| Forma de clusters | Esférica (Voronoi) | Arbitraria (densidad/conectividad) |")
        println(io, "| Entrada | Matriz de features | Grafo de similitud k-NN |")
        println(io, "")
    end
    println("  Louvain section → $path")
    return path
end
