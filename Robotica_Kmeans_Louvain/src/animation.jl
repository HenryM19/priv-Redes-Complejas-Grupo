...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  VISUALIZACIONES LOUVAIN — comunidades en grafo k-NN                           ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

# paleta amplia (Louvain puede generar más comunidades que K-Means)
const COMMUNITY_COLORS = [
    :seagreen, :darkorange, :tomato, :dodgerblue, :purple, :brown,
    :gold, :hotpink, :teal, :slateblue, :olive, :crimson,
]

"""
    plot_graph_communities(W, communities, Z, output_path; title) -> String

Dibuja el grafo k-NN con nodos posicionados por PCA 2D (`Z`, n×2) y coloreados
por comunidad Louvain. Las aristas se dibujan con alpha proporcional al peso.
PNG estático — el "mapa" del grafo de actuadores.
"""
function plot_graph_communities(
    W           ::Matrix{Float64},
    communities ::Vector{Int},
    Z           ::Matrix{Float64},
    output_path ::String;
    title       ::String = "Comunidades Louvain — Grafo k-NN de Actuadores"
)::String
    mkpath(dirname(output_path))
    n = size(W, 1)
    C = length(unique(communities))

    p = plot(
        xlabel = "PC1", ylabel = "PC2",
        title  = title, titlefontsize = 10,
        size   = (860, 620), dpi = 120,
        legend = :topright, grid = true, gridalpha = 0.25,
        framestyle = :box, background_color = :white
    )

    # aristas (dibujar primero, debajo de los nodos)
    wmax = maximum(W)
    for i in 1:n, j in i+1:n
        W[i, j] == 0.0 && continue
        a = 0.05 + 0.35 * (W[i, j] / wmax)
        plot!(p, [Z[i,1], Z[j,1]], [Z[i,2], Z[j,2]],
              color=:gray60, alpha=a, linewidth=0.6, label=false)
    end

    # nodos por comunidad
    for c in sort(unique(communities))
        idx = findall(==(c), communities)
        col = COMMUNITY_COLORS[(c - 1) % length(COMMUNITY_COLORS) + 1]
        scatter!(p, Z[idx,1], Z[idx,2],
                 color=col, markersize=6, markerstrokewidth=0.4,
                 alpha=0.85, label="Comunidad $c (n=$(length(idx)))")
    end

    savefig(p, output_path)
    println("  PNG grafo Louvain: $output_path")
    return output_path
end

"""
    animate_louvain(W, history, communities, Z, output_path; fps) -> String

GIF de la evolución multinivel de Louvain: cada frame muestra el grafo con la
partición de un nivel y su modularidad Q creciente. El último nivel = solución.
Si solo hay un nivel, genera un GIF de un frame (equivalente al PNG).
"""
function animate_louvain(
    W           ::Matrix{Float64},
    history     ::Vector,
    communities ::Vector{Int},
    Z           ::Matrix{Float64},
    ...
    n_levels = length(history)
    for (li, h) in enumerate(history)
        is_final = li == n_levels
        # usamos la partición final para colorear (la evolución de Q se muestra en título)
        comm = communities
        p = plot(
            xlims=(x_min,x_max), ylims=(y_min,y_max),
            xlabel="PC1", ylabel="PC2",
            title="Louvain — Nivel $(h.level)\nComunidades: $(h.n_comm)   |   Modularidad Q = $(h.Q)",
        ...
        for c in sort(unique(comm))
            ...
            scatter!(p, Z[idx,1], Z[idx,2], color=col,
                     markersize=6, markerstrokewidth=0.4, alpha=0.85, label=false)
        end
        frame(anim)
        if is_final
            for _ in 1:6; frame(anim); end
        end
    end

    gif(anim, output_path; fps=fps)
    println("  GIF Louvain: $output_path")
    return output_path
end
...
Panel comparativo: misma proyección PCA, K-Means a la izquierda y Louvain a la
derecha. La comparación visual central de las diapositivas.
"""
function plot_side_by_side(
    ...
    output_path    ::String;
    kmeans_title   ::String = "K-Means (geométrico)",
    louvain_title  ::String = "Louvain (comunidades)"
)::String
    mkpath(dirname(output_path))

    function panel(labels, ttl, palette)
        p = plot(xlabel="PC1", ylabel="PC2", title=ttl, titlefontsize=10,
                 legend=:topright, grid=true, gridalpha=0.25, framestyle=:box)
        for c in sort(unique(labels))
            idx = findall(==(c), labels)
            col = palette[(c-1) % length(palette) + 1]
            scatter!(p, Z[idx,1], Z[idx,2], color=col, markersize=5,
                     markerstrokewidth=0.3, alpha=0.7, label="C$c (n=$(length(idx)))")
        end
        return p
    end

    pk = panel(kmeans_labels,  kmeans_title,  CLUSTER_COLORS)
    pl = panel(louvain_labels, louvain_title, COMMUNITY_COLORS)
    plot(pk, pl, layout=(1,2), size=(1150,520), dpi=120,
         plot_title="K-Means vs Louvain — mismos actuadores")
    savefig(output_path)
    println("  PNG comparación: $output_path")
    return output_path
end
